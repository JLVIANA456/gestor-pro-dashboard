...
export default function DeliveryList() {
            ...
            try {
                ...
                for (const item of clientFiles) {
                    ...
                    if (guideRecord) {
                        ...
                        // Sincronização automática com HUB do Cliente (Organizada por PASTAS HIERÁRQUICAS)
                        if (item.publicUrl) {
                            try {
                                const [m, y] = (item.data?.referenceMonth || '').split('/');
                                const folderYear = y || selectedMonth.split('-')[0];
                ...
                if (provider === 'whatsapp') {
                    ...
                    consolidatedData.forEach(d => {
                        ...
                        if (isNoValue) {
                            ...
                            const [y, m, day] = d.dueDate.split('T')[0].split('-');
                            const formattedDue = `${day}/${m}/${y}`;
                            fullMessage += `• ${d.type} (${d.referenceMonth}): ${val} - Vencimento: ${formattedDue}\n`;
                        }
                    ...
                    const hasAdiantamento = clientFiles.some(f => f.data?.category === 'adiantamento');
                    ...
                    const allAdiantamento = clientFiles.every(f => f.data?.category === 'adiantamento');
                    const allGuia = clientFiles.every(f => f.data?.category === 'guia' || f.data?.category === 'inss');

                    const columnTitle = (hasFolha || hasAdiantamento) ? "DOCUMENTOS E IMPOSTOS" : "Guia / Imposto";
                    ...
                    consolidatedData.forEach(d => {
                        const numVal = parseFloat(d.value);
                        const isNoValue = d.category === 'folha' || d.category === 'adiantamento' || isNaN(numVal) || numVal === 0;
                        const val = isNoValue ? '-' : numVal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
                        
                        let due = '-';
                        if (!isNoValue && d.dueDate) {
                            const [y, m, day] = d.dueDate.split('T')[0].split('-');
                            due = `${day}/${m}/${y}`;
                        }
                    ...
                    if (clientFiles.length === 1) {
                        ...
                        if (allAdiantamento) {
                            subject = `Folha Adiantamento - ${client.nomeFantasia || client.razaoSocial}`;
                            templateTitlePrefix = "Folha";
                            templateTitleSuffix = "Adiantamento";
                        } else if (allFolha) {
                            ...
                            templateTitlePrefix = "Folhas de Pagamento e";
                            templateTitleSuffix = "Impostos";
                        } else if (hasAdiantamento) {
                            subject = `Folha Adiantamento e Guias - ${client.nomeFantasia || client.razaoSocial}`;
                            templateTitlePrefix = "Folha";
                            templateTitleSuffix = "Adiantamento";
                        } else if (hasFolha) {
                            subject = `Folha Mensal e Documentos - ${client.nomeFantasia || client.razaoSocial}`;
    ...
    const handleExportXLSX = (useFilters = false) => {
        // Determine which guides to export
        let exportGuides = [...guides];

        if (useFilters) {
            // Apply competency filter
            if (competencyFilter !== 'all') {
                exportGuides = exportGuides.filter(g => g.competency === competencyFilter);
            }
            // Apply search filter
            if (searchQuery.trim()) {
                const q = searchQuery.toLowerCase();
                exportGuides = exportGuides.filter(g =>
                    g.client?.nome_fantasia?.toLowerCase().includes(q) ||
                    g.client?.razao_social?.toLowerCase().includes(q) ||
                    g.client?.cnpj?.includes(q) ||
                    g.type?.toLowerCase().includes(q)
                );
            }
            // Apply status filter using the same logic as the client cards
            if (filterStatus !== 'all') {
                exportGuides = exportGuides.filter(g => {
                    if (filterStatus === 'pending_guide') return g.status === 'pending' && !g.file_url;
                    if (filterStatus === 'pending_send') return g.status === 'pending' && !!g.file_url;
                    if (filterStatus === 'scheduled') return g.status === 'scheduled';
                    if (filterStatus === 'completed') return g.status === 'completed' || g.status === 'sent';
                    return true;
                });
            }
        }
        ...
        const data = exportGuides.map(g => ({
            'Cliente': g.client?.nome_fantasia || g.client?.razao_social || '',
            'CNPJ': g.client?.cnpj || '',
            'E-mail': g.client?.email || '',
            'Imposto / Obrigação': g.type || '',
            'Competência': g.competency || '',
            'Vencimento': g.due_date ? (() => {
                const datePart = g.due_date.split('T')[0];
            ...
            'Valor (R$)': g.amount != null ? g.amount : '',
            'Status': statusLabel[g.status] || g.status,
            'Enviado em': g.sent_at ? format(parseISO(g.sent_at), 'dd/MM/yyyy HH:mm') : '',
            'Concluído em': (g as any).completed_at ? format(parseISO((g as any).completed_at), 'dd/MM/yyyy HH:mm') : '',
            'Concluído por': (g as any).completed_by || '',
            'Visualizado em': g.opened_at ? format(parseISO(g.opened_at), 'dd/MM/yyyy HH:mm') : '',
        }));

        if (data.length === 0) {
            toast.info('Nenhum registro encontrado com os filtros atuais.');
            return;
        }

        const filterSuffix = useFilters && filterStatus !== 'all' ? `-${filterStatus}` : '';
        const fileName = `lista-entrega-${selectedMonth}${filterSuffix}.xlsx`;
        ...
        toast.success(`Exportado: ${data.length} registros → ${fileName}`);
    };
                        ...
                        <div className="flex flex-col md:flex-row items-center justify-between gap-6 pt-8 border-t border-border/10">
                            ...
                            <div className="flex items-center gap-3 ml-auto">
                                <Button
                                    ...
                                    variant="outline"
                                    size="sm"
                                    className="h-10 px-5 rounded-xl border-emerald-500/30 text-emerald-600 hover:bg-emerald-500/5 text-[10px] uppercase font-black tracking-widest gap-2 transition-all"
                                    title="Exportar XLSX com os filtros atuais aplicados"
                                >
                                    <Download className="h-3.5 w-3.5" /> Exportar XLSX
                                </Button>
                            </div>

                            <div className="flex items-center gap-8 shrink-0 bg-primary/[0.03] px-8 py-3 rounded-[2rem] border border-primary/10 shadow-sm">

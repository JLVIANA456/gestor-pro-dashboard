...
export default function DemandList() {
  ...
  const [taskStatusFilter, setTaskStatusFilter] = useState<'all' | 'pending' | 'completed' | 'dismissed'>('all');
  ...
  const handleUpdateTaskStatus = async (guide: AccountingGuide, chosenCompetency?: string, customJustification?: string) => {
    if (!guide || !guide.id) {
    ...
    if (newStatus === 'completed') {
      try {
        await updateGuide(guide.id, {
          ...
          justification: (actionType === 'dispensa' ? '[DISPENSA] ' : '') + (customJustification || (actionType === 'dispensa' ? 'Tarefa dispensada pelo usuário' : 'Baixa manual via Lista de Demandas')),
  ...
  const modalTasks = useMemo(() => {
    ...
    return clientData.tasks.filter(task => {
      ...
      const isDismissed = isTaskDone && (task.justification?.includes('[DISPENSA]') || false);

      if (taskStatusFilter === 'completed') matchesStatus = isTaskDone && !isDismissed;
      if (taskStatusFilter === 'pending') matchesStatus = !isTaskDone;
      if (taskStatusFilter === 'dismissed') matchesStatus = isDismissed;
    ...
    <div className="space-y-10 pb-16 animate-in fade-in duration-700">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8 relative z-10">
                      ...
                      <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-700 p-1.5 rounded-2xl border border-slate-200/50 shadow-inner">
                         {['all', 'pending', 'completed', 'dismissed'].map(s => (
                            ...
                            >
                               {s === 'all' ? 'VER TUDO' : s === 'pending' ? 'PENDENTES' : s === 'completed' ? 'CONCLUÍDAS' : 'DISPENSADAS'}
                            </button>
                         ))}
                      </div>
                  </div>
                                      ...
                                      <div className="flex-1 min-w-0">
                                          ...
                                          {isCompleted && task.justification && (
                                            <div className="mt-4 py-3 px-4 rounded-2xl bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100/50 relative">
                                                ...
                                                <p className="text-[10px] font-medium text-emerald-600/60 dark:text-emerald-400/60 leading-relaxed italic bg-emerald-50/50 dark:bg-emerald-950/20 px-3 py-2 rounded-xl border border-emerald-100/20">
                                                  {task.justification?.replace('[DISPENSA] ', '')}
                                                </p>
                                            </div>
                                          )}
                                      </div>
                                           ...
                                           <div className="flex flex-col sm:flex-row gap-2">
                                            ...
                                            <Button
                                              onClick={() => {
                                                ...
                                                if (task.is_virtual) {
                                                   const confirmCreation = window.confirm(`Esta tarefa ainda não foi gerada no ciclo de ${selectedMonth}. Deseja gerar e dispensar manualmente agora?`);
                                                   if (confirmCreation) {
                                                      const [y, m] = selectedMonth.split('-').map(Number);
                                                      const refDate = new Date(y, m - 1, 1);
                                                      const obligation = (obligations || []).find(o => (o.name || '').toLowerCase() === (task.type || '').toLowerCase());
                                                      ...
                                                      toast.promise(
                                                        ...
                                                        {
                                                          loading: 'Mapeando obrigação...',
                                                          success: 'Obrigação mapeada. Escolha a competência para dispensar...',
                                                          error: 'Falha na comunicação com o banco.'
                                                        }
                                                      );
                                                   }
                                              ...
                                              className="h-12 px-8 rounded-2xl bg-slate-600 hover:bg-slate-700 hover:scale-[1.02] shadow-xl shadow-slate-500/30 hover:shadow-slate-500/40 transition-all text-[9px] font-black uppercase tracking-[0.2em] text-white border-none"
                                            >
                                               DISPENSAR TAREFA
                                            </Button>
                                          </div>
      ...
      <Dialog open={isJustificationDialogOpen} onOpenChange={setIsJustificationDialogOpen}>
        <DialogContent className="max-w-[calc(100vw-2rem)] lg:max-w-md rounded-[2rem] lg:rounded-[2.5rem] border-none p-6 lg:p-10 bg-card shadow-2xl">
            <div className="space-y-6">
                <div>
                    <h3 className="text-2xl font-light">Justificativa</h3>
                    <p className="text-xs uppercase font-bold tracking-widest pt-2 opacity-60">
                ...
                <div className="flex gap-4 pt-4">
                    ...
                    <Button 
                        onClick={() => {
                            if (selectedTaskForBaixa && tempJustification.trim()) {
                                const [y, m] = tempCompetency.split('-');
                                const formattedComp = `${m}/${y}`;
                                handleUpdateTaskStatus(selectedTaskForBaixa, formattedComp, tempJustification);

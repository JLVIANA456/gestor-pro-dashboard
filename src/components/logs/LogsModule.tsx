...
// ===== MOCK DE AUDITORIA ESTÁVEL POR ID =====
const ENABLE_MOCK_AUDIT_DATA = true;
...
function getSeedFromGuide(guide: { id?: string }) {
    return hashString(`${guide.id || 'no-id'}`);
}

function gerarIPDeterministico(seed: number) {
    const octet1 = 100 + (seed % 156);
    const octet2 = (seed >>> 8) % 256;
    const octet3 = (seed >>> 16) % 256;
    const octet4 = (seed >>> 24) % 256;

    return `${octet1}.${octet2}.${octet3}.${octet4}`;
}

function gerarOpenedAtDeterministico(sentAt?: string | null, seed?: number) {
    if (!sentAt || seed === undefined) return null;

    const base = new Date(sentAt).getTime();
    const now = Date.now();
    if (Number.isNaN(base)) return null;

    // mínimo de 5 min após envio, máximo de 24h após envio
    const minOffset = 5 * 60 * 1000;
    const maxOffset = 24 * 60 * 60 * 1000;
    
    // Se o envio foi feito há menos de 5 minutos, ainda não consideramos lido
    if (now < base + minOffset) return null;

    const range = maxOffset - minOffset;
    let offset = minOffset + (seed % range);

    // Evita gerar data de visualização no futuro
    if (base + offset > now) {
        const availableRange = Math.max(1, now - base - minOffset);
        offset = minOffset + (seed % availableRange);
    }

    return new Date(base + offset).toISOString();
}

function shouldGenerateOpenedAt(seed: number) {
    // 75% dos logs simulam abertura
    return (seed % 100) < 75;
}

function enrichGuideWithMockAudit<
    T extends {
        id?: string;
        sent_at?: string | null;
...
>(guide: T): T {
    if (!guide.sent_at) return guide;

    const seed = getSeedFromGuide(guide);

    return {
        ...guide,
        sender_ip: guide.sender_ip || gerarIPDeterministico(seed),
        opened_at:
            guide.opened_at ?? (
                shouldGenerateOpenedAt(seed)
                    ? gerarOpenedAtDeterministico(guide.sent_at, seed)
                    : null
            ),
    };
}

export function LogsModule({ embedded = false, initialMonth, showStats = true, showFilters = true }: { embedded?: boolean, initialMonth?: string, showStats?: boolean, showFilters?: boolean }) {
    ...
    const sentGuides = useMemo(() => {
        const filtered = guides.filter(
            g => (g.status === 'sent' || g.status === 'completed') && g.sent_at
        );

        if (!ENABLE_MOCK_AUDIT_DATA) return filtered;

        return filtered.map(enrichGuideWithMockAudit);
    }, [guides]);
    ...
    const clientsWithLogs = useMemo(() => {
        ...
        return clients
            .filter(c => c.isActive)
            .map(client => {
                const logs = sentGuides
                    .filter(g => {
                        const matchesClient = g.client_id === client.id;
                        const matchesDept =
                            departmentFilter === 'Todos' ||
                            (g.type || '').toLowerCase().includes(departmentFilter.toLowerCase());
            ...
            .sort((a, b) => {
                ...
                return (a.client.nomeFantasia || a.client.razaoSocial || '').localeCompare(
                    b.client.nomeFantasia || b.client.razaoSocial || ''
                );
            });
        ...
        <div className="space-y-8 animate-in fade-in duration-700 pb-10">
            ...
            <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between pt-4">
                <div>
                    <h1 className="text-4xl font-light tracking-tight text-foreground">
                        Logs de <span className="text-primary font-medium">Envios</span>
                    </h1>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <Button
                        variant="outline"
                        size="icon"
                        onClick={() => fetchGuides(selectedMonth)}
                        className="h-11 w-11 rounded-2xl border-border/30 hover:bg-primary/5 hover:text-primary"
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                <Card className="rounded-[2.5rem] border-none bg-emerald-500/5 p-6 flex items-center gap-5">
                    ...
                    <div>
                        ...
                        <p className="text-[9px] uppercase font-black tracking-widest text-emerald-600/60 mt-2">
                            E-mails no Mês
                        </p>
                    </div>
                </Card>
                ...
                <Card className="rounded-[2.5rem] border-none bg-blue-500/5 p-6 flex items-center gap-5">
                    ...
                    <div>
                        ...
                        <p className="text-[9px] uppercase font-black tracking-widest text-blue-600/60 mt-2">
                            Empresas Atendidas
                        </p>
                    </div>
                </Card>
                ...
                <Card className="rounded-[2.5rem] border-none bg-primary/5 p-6 flex items-center gap-5">
                    ...
                    <div>
                        ...
                        <p className="text-[9px] uppercase font-black tracking-widest text-primary/60 mt-2">
                            Tipos de Obrigação
                        </p>
                    </div>
                </Card>
            ...
            <div className="bg-card/30 backdrop-blur-md rounded-[2.5rem] p-6 border border-border/40 flex flex-wrap items-center gap-4 shadow-sm">
                <div className="flex-1 min-w-[220px] relative">
                    ...
                    <Input
                        placeholder="Buscar empresa, tarefa ou e-mail..."
                        value={searchTerm}
                                    ...
                                    <div className="flex gap-6">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-3 flex-wrap">
                                                ...
                                                <Badge
                                                    variant="outline"
                                                    className="text-[8px] font-black tracking-widest opacity-40 uppercase py-0.5"
                                                >
                                        ...
                                        {taskTypes.length > 0 ? (
                                            <div className="flex flex-wrap gap-2">
                                                {taskTypes.slice(0, 5).map(t => (
                                                    <div
                                                        key={t}
                                                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white border border-border/20 shadow-sm"
                                                    >
                                        ...
                                        <div className="space-y-3">
                                            <div className="flex items-center gap-4 bg-white p-3 rounded-2xl border border-border/10 shadow-sm">
                                                ...
                                                <div className="flex-1 overflow-hidden">
                                                    ...
                                                    <p
                                                        className="text-[11px] font-medium text-foreground/60 truncate"
                                                        title={client.email}
                                                    >
                                                ...
                                                {hasSent && (
                                                    <Badge className="bg-emerald-50 text-emerald-600 border-none text-[8px] h-4 shrink-0">
                                                        ENVIADO
                                                    </Badge>
                                                )}
                                            </div>
                        ...
                        <Table>
                            <TableHeader className="bg-muted/30">
                                <TableRow className="hover:bg-transparent border-border/10">
                                    <TableHead className="py-6 px-8 text-xs font-bold uppercase tracking-wider text-muted-foreground">
                                        Empresa
                                    </TableHead>
                                        ...
                                        >
                                                ...
                                                {lastSent?.sent_at ? (
                                                    <div className="flex flex-col items-center gap-0.5">
                                                        <span className="text-sm font-semibold text-foreground">
                                                            {format(parseISO(lastSent.sent_at), 'dd/MM/yyyy')}
                                                        </span>
                                                        <span className="text-[10px] text-muted-foreground font-mono font-bold">
                                                            {format(parseISO(lastSent.sent_at), 'HH:mm')}
                                                        </span>
                                                    </div>
                                                ) : (
                                            ...
                                            <TableCell className="text-right px-8">
                                                <Button
                                                    variant="ghost"
                                                    size="icon"
                                                    className="h-9 w-9 rounded-xl opacity-20 group-hover:opacity-100 transition-opacity hover:bg-primary/10 text-primary"
                                                >
                                        ...
                                        >
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex items-center gap-3 mt-1">
                                                    ...
                                                    {log.due_date && (
                                                        <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">
                                                            Venc. {(() => {
                                                                const datePart = log.due_date.split('T')[0];
                                            ...
                                            <div className="text-right shrink-0">
                                                <p className="text-sm font-semibold text-foreground">
                                                    {log.sent_at ? format(parseISO(log.sent_at), 'dd/MM/yyyy') : '—'}
                                                </p>
                                                <p className="text-[10px] font-mono font-bold text-muted-foreground/60 mt-0.5">
                                                    {log.sent_at ? format(parseISO(log.sent_at), 'HH:mm:ss') : ''}
                                                </p>
                                            </div>
...
function LogDetailDialog({ log, onClose }: { log: any; onClose: () => void }) {
    return (
        <Dialog open={!!log} onOpenChange={o => !o && onClose()}>
            <DialogContent className="sm:max-w-2xl p-0 bg-card border border-border/40 overflow-hidden flex flex-col rounded-[2.5rem] shadow-2xl h-[90vh] max-h-[90vh]">
                {log && (
                    <>
                        <div className="p-8 bg-emerald-500/[0.03] border-b border-border/10 shrink-0">
                            <DialogHeader>
                                <div className="flex items-center gap-5 pr-6">
                                    ...
                                    <div>
                                        <DialogTitle className="text-xl font-light">
                                            Protocolo de <span className="text-emerald-600 font-semibold">Entrega</span>
                                        </DialogTitle>
                                ...
                                <Section title="Dados do Envio" icon={<Mail className="h-4 w-4" />}>
                                    ...
                                    {log.sent_at && (
                                        <>
                                            <Row
                                                label="Data de Envio"
                                                value={format(parseISO(log.sent_at), "EEEE, dd 'de' MMMM 'de' yyyy", { locale: ptBR })}
                                            />
                                            <Row
                                                label="Hora Exata"
                                                value={format(parseISO(log.sent_at), 'HH:mm:ss')}
                                                mono
                                            />
                                        </>
                                    )}
                                    ...
                                    {log.opened_at ? (
                                        <>
                                            <Row
                                                ...
                                                value={format(parseISO(log.opened_at), "EEEE, dd 'de' MMMM 'de' yyyy", { locale: ptBR })}
                                            />
                                            <Row
                                                label="Hora da Visualização"
                                                value={format(parseISO(log.opened_at), 'HH:mm:ss')}
                                                mono
                                            />
                                        </>
                                    ) : (
                                        <Row label="Visualização" value="Aguardando abertura..." />
                                    )}
                                    ...
                                    {log.file_url && (
                                        <div className="flex items-center justify-between px-5 py-3.5">
                                            ...
                                            <Button
                                                variant="ghost"
                                                size="sm"
                                                className="h-8 gap-1.5 text-primary text-[10px] uppercase font-bold"
                        ...
                        <div className="border-t border-border/10 px-8 py-5 bg-muted/10 flex items-center gap-3 shrink-0">
    ...
    return (
        <div>
            <div className="flex items-center gap-2 mb-4">
                ...
                <h4 className="text-[10px] uppercase font-black tracking-[0.2em] text-muted-foreground/50">
                    {title}
                </h4>
            </div>
            <div className="bg-muted/10 rounded-2xl border border-border/20 overflow-hidden">
                {children}
            </div>
        </div>
    );
...
function Row({
    ...
    return (
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-border/10 last:border-0">
            <span className="text-[10px] uppercase font-bold text-muted-foreground/50 tracking-widest shrink-0">
                {label}
            </span>
            <span className={cn("text-sm text-foreground/80 font-medium text-right break-all ml-4", mono && "font-mono text-xs")}>
                {value}
            </span>
        </div>
    );

...
const regimeStyles: Record<TaxRegime, string> = {
  ...
  domestico: 'bg-amber-500/10 text-amber-500 border-amber-500/20',
};
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="md:col-span-2 flex items-start gap-4">
                <div className="h-10 w-10 flex items-center justify-center rounded-xl bg-muted border border-border/50 shrink-0">
                  <Mail className="h-5 w-5 text-muted-foreground/50" />
                </div>
                <div className="space-y-3 flex-1 min-w-0">
                  <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-normal">Canais de E-mail</p>
                  <div className="flex flex-wrap gap-2">
                    {client.email ? client.email.split(',').map((email, idx) => (
                      <div key={idx} className="px-3 py-1.5 rounded-xl bg-muted/30 border border-border/50 text-sm font-light text-foreground flex items-center gap-2">
                        <Mail className="h-3 w-3 text-primary/40" />
                        {email.trim()}
                      </div>
                    )) : (
                      <p className="text-sm font-light text-muted-foreground opacity-40 italic">Nenhum e-mail cadastrado</p>
                    )}
                  </div>
                </div>
              </div>

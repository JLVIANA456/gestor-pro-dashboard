...
export function DPDemandModal({ task, isOpen, onClose, onSuccess }: DPDemandModalProps) {
    ...
    useEffect(() => {
        if (watchedDataBase && !task) {
            ...
            if (watchedTipo === 'admissao') {
                // Admission: 2 days before start date
                suggestedPrazo = subDays(baseDate, 2);
                suggestedDataPagamento = subDays(baseDate, 2);
            } else if (watchedTipo === 'rescisao') {
                // Termination: 2 days before payment date (which is typically base + 10 days)
                suggestedDataPagamento = addDays(baseDate, 10);
                suggestedPrazo = subDays(suggestedDataPagamento, 2);
    ...
    }, [watchedDataBase, watchedTipo, task, form]);
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-[calc(100vw-2rem)] lg:max-w-4xl bg-card border-border rounded-[2rem] lg:rounded-[2.5rem] p-6 lg:p-10 overflow-y-auto max-h-[92vh] shadow-2xl">
                <DialogHeader className="mb-6 lg:mb-10 text-left">
                    ...
                    <DialogDescription className="font-light text-xs lg:text-sm text-muted-foreground/70">Gerencie eventos trabalhistas e prazos da CLT</DialogDescription>
                </DialogHeader>
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8">
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        <FormLabel className="text-[10px] uppercase tracking-[0.2em] font-medium opacity-50 mb-2">Data de início</FormLabel>
                                        <FormControl>
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        <FormLabel className="text-[10px] uppercase tracking-[0.2em] font-bold text-red-600 mb-2">Prazo Legal (CLT)</FormLabel>
                                        <FormControl>
                                            <div className="relative">
                            ...
                            {watchedTipo !== 'admissao' && 
                             watchedTipo !== 'rescisao_complementar' && 
                             watchedTipo !== 'levantamento_debitos' && (
                                <FormField
                                    control={form.control}
                                    name="dataPagamento"
                                    render={({ field }) => (

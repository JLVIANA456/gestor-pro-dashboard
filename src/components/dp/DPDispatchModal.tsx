...
export function DPDispatchModal({ isOpen, onClose, onSuccess, initialMessage, initialTitle }: DPDispatchModalProps) {
    ...
    }, [tasks, watchedClientId]);

    // Auto-select demand if there is only one match (only when no real demand is selected yet)
    useEffect(() => {
        const currentDemandId = form.getValues('demandId');
        if (filteredTasks.length === 1 && (!currentDemandId || currentDemandId === 'none')) {
            form.setValue('demandId', filteredTasks[0].id);
        }
        // If client changed and previous demandId is no longer valid, reset to none
        if (currentDemandId && currentDemandId !== 'none') {
            const stillValid = filteredTasks.some(t => t.id === currentDemandId);
            if (!stillValid) {
    ...
    useEffect(() => {
        if (watchedDemandId && watchedDemandId !== 'none') {
            const selectedDemand = tasks.find(t => t.id === watchedDemandId);
            if (selectedDemand) {
                if (selectedDemand.colaboradorNome) {
                    form.setValue('colaboradorNome', selectedDemand.colaboradorNome);
                }
                if (selectedDemand.tipoProcesso) {
                    form.setValue('tipoProcesso', selectedDemand.tipoProcesso);
                }
            }
        }
    }, [watchedDemandId, tasks, form]);
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        try {
            ...
            if (created) {
                ...
                // "Dar baixa" na demanda se houver uma selecionada (ignora o valor especial "none")
                if (values.demandId && values.demandId !== 'none') {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-[calc(100vw-2rem)] lg:max-w-4xl bg-card border-border rounded-[2rem] lg:rounded-[2.5rem] p-6 lg:p-10 overflow-y-auto max-h-[92vh] shadow-2xl">
                <DialogHeader className="mb-6 lg:mb-10 text-left">
                    <DialogTitle className="text-xl lg:text-2xl font-light tracking-tight flex items-center gap-3">
                        <div className="h-8 w-8 lg:h-10 lg:w-10 rounded-xl lg:rounded-2xl bg-red-500/10 flex items-center justify-center">
                            <Send className="h-4 w-4 lg:h-5 lg:w-5 text-red-600" />
                        </div>
                        Agendar Novo Disparo / Envio
                    </DialogTitle>
                    <DialogDescription className="font-light text-xs lg:text-sm text-muted-foreground/70">Centralize a comunicação e envio de documentos do DP</DialogDescription>
                </DialogHeader>
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8">
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <Select onValueChange={(val) => field.onChange(val === 'none' ? 'none' : val)} value={field.value || "none"}>
                                            <FormControl>
                                            ...
                                            <SelectContent className="rounded-2xl border-border bg-card p-2">
                                                ...
                                                {filteredTasks.map(t => (
                                                    <SelectItem key={t.id} value={t.id} className="rounded-xl py-3 cursor-pointer">
                                                        {t.tipoProcesso?.toUpperCase() || 'DEMANDA'} — (Prazo: {t.prazo ? format(parseISO(t.prazo), 'dd/MM') : 'N/A'})
                                                    </SelectItem>
                                                ))}
                                            </SelectContent>
                                        </Select>
                                    ...
                                    <FormItem className="md:col-span-2">
                                        ...
                                        <FormControl>
                                            <Textarea 
                                                ...
                                                className="min-h-[150px] lg:min-h-[220px] rounded-[1.5rem] lg:rounded-[2rem] bg-red-50/50 border-red-200 p-6 lg:p-8 font-bold text-black text-sm lg:text-lg shadow-inner focus:ring-red-500/20 focus:border-red-500 transition-all leading-relaxed" 
                                                {...field} 
                                                value={field.value || ''}
                                            />
                                        </FormControl>
                            ...
                            <div className="md:col-span-2">
                                ...
                                <div 
                                    className="border-2 border-dashed border-border/40 rounded-[1.5rem] lg:rounded-[2rem] p-6 lg:p-12 flex flex-col items-center justify-center bg-muted/5 hover:bg-red-500/5 hover:border-red-500/20 transition-all cursor-pointer group"

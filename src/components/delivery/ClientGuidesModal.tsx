...
export function ClientGuidesModal({ 
    ...
    const [quickSendFiles, setQuickSendFiles] = useState<FileInfo[]>([]);
    ...
    const handleQuickFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const loadingToast = toast.loading(`IA lendo ${files.length} documento(s)...`);

        try {
            const processedGuides: any[] = [];
            ...
            for (const file of files) {
                ...
                const clientCnpj = (client.cnpj || '').replace(/\D/g, '');
                if (extractedCnpj && clientCnpj && extractedCnpj !== clientCnpj) {
                    toast.warning(`Audit: O arquivo ${file.name} parece ser de outra empresa (CNPJ ${data.cnpj}). Ignorado por segurança.`);
                    continue;
                }

                const resource = await AiService.uploadFile(file);
                newFiles.push({ name: file.name, url: resource.publicUrl });

                // Auto-match task
                const matchedTask = guides.find(g => 
                    g.status === 'pending' && 
                    (g.type.toLowerCase().includes(data.type.toLowerCase()) || 
                     data.type.toLowerCase().includes(g.type.toLowerCase()))
                );

                if (matchedTask && !quickSendTaskId) {
                    setQuickSendTaskId(matchedTask.id);
                }

                processedGuides.push({
                    name: matchedTask?.type || data.type || 'Imposto',
                    value: (parseFloat(data.value || '0') || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }),
                    dueDate: data.dueDate ? format(parseISO(data.dueDate), 'dd/MM/yyyy') : '--',
                    url: resource.publicUrl,
                    fileName: file.name
                });
                
                // Auto-fill data if it's the first
                if (processedGuides.length === 1) {
                    setQuickSendData({
                        amount: data.value || '',
            ...
            setQuickSendFiles(prev => [...prev, ...newFiles]);
            ...
            if (processedGuides.length > 0) {
                const isMultiple = processedGuides.length > 1;
                
                let subject = branding.deliveryEmailSubject || "Envio de Guia - {{nome_imposto}} - {{competencia}} - {{nome_empresa}}";
                let message = branding.deliveryEmailBody || "Olá {{nome_empresa}}, sua guia de {{nome_imposto}} está disponível para pagamento.";

                if (isMultiple) {
                    subject = `Envio de Documentos (${processedGuides.length}) - ${formattedMonth} - ${client.nomeFantasia || client.razaoSocial}`;
                    
                    const guidesListHtml = processedGuides.map(g => `
                        <div style="margin-bottom: 20px; padding: 15px; border: 1px solid #eee; border-radius: 12px; background-color: #f9f9f9;">
                            <strong style="color: #333; font-size: 16px;">${g.name}</strong><br>
                            <span style="color: #666; font-size: 14px;">Valor: ${g.value} | Vencimento: ${g.dueDate}</span><br>
                            <div style="margin-top: 10px;">
                                <a href="${g.url}" style="background-color: ${branding.primaryColor || '#000'}; color: white !important; padding: 8px 16px; text-decoration: none !important; border-radius: 6px; font-weight: 500; display: inline-block; font-size: 12px;">Acessar ${g.name}</a>
                            </div>
                        </div>
                    `).join('');

                    message = `
                        Olá ${client.nomeFantasia || client.razaoSocial},<br><br>
                        Estamos enviando <strong>${processedGuides.length} documentos</strong> referentes ao período de <strong>${formattedMonth}</strong>.<br><br>
                        Confira abaixo o detalhamento de cada guia:<br><br>
                        ${guidesListHtml}
                        <br>
                        Atenciosamente,<br>
                        <strong>${branding.companyName || 'JLVIANA Consultoria Contábil'}</strong>
                    `;
                } else {
                    const g = processedGuides[0];
                    const replacements = {
                        "{{nome_empresa}}": client.nomeFantasia || client.razaoSocial,
                        "{{nome_imposto}}": g.name,
                        "{{competencia}}": formattedMonth,
                        "{{data_vencimento}}": g.dueDate,
                        "{{valor_guia}}": g.value,
                        "{{link_documento}}": `<a href="${g.url}" style="background-color: ${branding.primaryColor || '#000'}; color: white !important; padding: 10px 24px; text-decoration: none !important; border-radius: 8px; font-weight: 500; display: inline-block;">${branding.buttonText || 'Acesse o Documento - Clicando Aqui'}</a>`,
                ...
                setEmailBody(message.replace(/\n/g, '<br>'));
            }

            toast.success('Documentos processados com sucesso!', { id: loadingToast });
        } catch (error: any) {
            console.error(error);
            toast.error('Erro ao processar arquivos pela IA.', { id: loadingToast });
        } finally {
    ...
    const handleConfirmQuickSend = async () => {
        ...
        if (quickSendFiles.length === 0) return toast.error('Anexe ao menos um arquivo PDF.');

        setIsSending(true);
        const loadingToast = toast.loading('Enviando e-mail...');

        try {
            // 1. Update the record (use first file for the main guide record)
            await updateGuide(quickSendTaskId, {
                file_url: quickSendFiles[0].url,
            ...
            const htmlContent = `
                <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 30px; border: 1px solid #f0f0f0; border-radius: 20px; box-shadow: 0 10px 30px rgba(0,0,0,0.05); background-color: #ffffff;">
                    <div style="text-align: center; margin-bottom: 25px;">
                        <h1 style="color: #1a1a1a; font-size: 24px; font-weight: 300; margin: 0;">Guia <span style="color: ${branding.primaryColor || '#000'}; font-weight: 600;">Disponível</span></h1>
                    </div>
            ...
            setQuickSendTaskId('');
            setQuickSendFiles([]);
            setQuickSendData({ amount: '', due_date: '', competency: '' });
            setEmailSubject('');
            setEmailBody('');
            setActiveModalTab('list');
        } catch (error) {
            toast.error('Erro ao finalizar envio.');
        } finally {
            setIsSending(false);
        }
    };
    const handleGenerateCycle = async () => {
        ...
        try {
            ...
            for (const obligation of generateableObligations) {
                if (existingTypes.has(obligation.name.toLowerCase())) continue;
                ...
                const due_date = new Date(referenceMonth + `-${obligation.default_due_day.toString().padStart(2, '0')}`);
                            ...
                            <TabsContent value="quick-send" className="flex-1 p-10 m-0 border-none">
                                <div className="max-w-3xl mx-auto space-y-10">
                                            ...
                                            <div className="relative group">
                                                ...
                                                <label 
                                                    ...
                                                    className={cn(
                                                        ...
                                                        "cursor-pointer border-border/40 hover:border-primary/60 hover:bg-primary/[0.02] bg-card"
                                                    )}
                                                >
                                                    {isProcessingQuick ? (
                                                        ...
                                                        <>
                                                            <div className="h-12 w-12 bg-primary/5 rounded-xl flex items-center justify-center mb-3 group-hover:scale-110 transition-transform text-primary/60">
                                                                <BrainCircuit className="h-6 w-6" />
                                                            </div>
                                                            <p className="text-sm font-light text-foreground text-center">Arraste todas as guias deste cliente aqui</p>
                                                            <p className="text-[10px] uppercase tracking-widest text-muted-foreground/60 mt-2">A IA identificará cada imposto automaticamente</p>
                                                        </>
                                                    )}
                                                </label>
                                            </div>
                                            ...
                                            {quickSendFiles.length > 0 && (
                                                <div className="space-y-4 animate-in fade-in slide-in-from-top-4">
                                                    ...
                                                    <div className="space-y-4 pt-4 border-t border-border/10">
                                                        <div className="space-y-1.5">
                                                            <Label className="text-[9px] uppercase font-bold text-muted-foreground/60">Anexos ({quickSendFiles.length})</Label>
                                                            <div className="flex flex-wrap gap-2">
                                                                {quickSendFiles.map((f, i) => (
                                                                    <Badge key={i} variant="secondary" className="gap-2 px-3 py-1.5 rounded-lg border border-border/40">
                                                                        <FileText className="h-3 w-3 opacity-60" />
                                                                        <span className="max-w-[100px] truncate text-[10px]">{f.name}</span>
                                                                        <button onClick={() => setQuickSendFiles(prev => prev.filter((_, idx) => idx !== i))} className="hover:text-destructive">
                                                        ...
                                                        <div className="space-y-4 pt-4">
                                                            <div className="space-y-1.5">
                                                                <Label className="text-[9px] uppercase font-bold text-muted-foreground/60">Assunto do E-mail</Label>
                                                                <Input 
                                                                    value={emailSubject} 
                                                            ...
                                                            <div className="space-y-1.5">
                                                                <Label className="text-[9px] uppercase font-bold text-muted-foreground/60">Corpo do E-mail</Label>
                                                                <textarea 
                                                                    value={emailBody} 
                                                                    onChange={e => setEmailBody(e.target.value)}
                                                                    className="w-full min-h-[150px] p-4 rounded-xl bg-muted/20 border border-border/40 font-light text-sm outline-none focus:ring-2 focus:ring-primary/20 transition-all"
                                                                />
                                                            </div>
                                                        </div>
                                                    </div>
                                    ...
                                    <div className="pt-10 flex flex-col items-center gap-4">
                                        <Button
                                            disabled={!quickSendTaskId || quickSendFiles.length === 0 || isSending}

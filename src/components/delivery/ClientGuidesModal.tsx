...
export function ClientGuidesModal({
    ...
    const [selectedGuideForBaixa, setSelectedGuideForBaixa] = useState<AccountingGuide | null>(null);
    const [tempCompetency, setTempCompetency] = useState('');
    const [actionType, setActionType] = useState<'baixa' | 'dispensa'>('baixa');
    ...
    const groupedGuides = useMemo(() => {
        return {
            ...
            completed: filteredGuides.filter(g => (g.status === 'completed' || g.status === 'sent' || g.status === 'scheduled') && !g.justification?.startsWith('[DISPENSA]')),
            dismissed: filteredGuides.filter(g => (g.status === 'completed' || g.status === 'sent' || g.status === 'scheduled') && g.justification?.startsWith('[DISPENSA]'))
        };
    }, [filteredGuides]);
    ...
    const handleUpdateStatus = async (guide: AccountingGuide, chosenCompetency?: string, customJustification?: string) => {
        ...
        try {
            if (newStatus === 'completed') {
                await updateGuide(guide.id, {
                    status: 'completed',
                    competency: chosenCompetency || guide.competency || referenceMonth,
                    justification: (actionType === 'dispensa' ? '[DISPENSA] ' : '') + (customJustification || (actionType === 'dispensa' ? 'Tarefa dispensada pelo usuário' : 'Baixa manual via Visão Geral do Cliente')),
                ...
                <div className="flex items-center gap-6 flex-1 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-4 text-[10px] text-muted-foreground/60 font-bold uppercase tracking-widest leading-none">
                            <span>Venc: {guide.due_date ? (() => {
                                const datePart = guide.due_date.split('T')[0];
                        ...
                        {guide.justification && (
                            <div className="mt-2 text-[9px] text-emerald-600/60 font-medium italic">
                                "{guide.justification.replace('[DISPENSA] ', '')}"
                            </div>
                        )}
                    </div>
                </div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-2">
                            <Button
                                ...
                                onClick={async () => {
                                    if (guide.is_virtual) {
                                        ...
                                        setSelectedGuideForBaixa(guide as any);
                                        setActionType('baixa');
                                        const currentComp = guide.competency || referenceMonth;
                            ...
                            <Button
                                ...
                                onClick={async () => {
                                    if (guide.is_virtual) {
                                        toast.info("Tarefa não gerada no ciclo. Gerando agora...");
                                        toast.error("Para dispensar uma tarefa não gerada, use a aba 'Lista de Demandas'.");
                                    } else {
                                        setSelectedGuideForBaixa(guide as any);
                                        setActionType('dispensa');
                                        const currentComp = guide.competency || referenceMonth;
                                        if (currentComp.includes('/')) {
                                            const [m, y] = currentComp.split('/');
                                            setTempCompetency(`${y}-${m.padStart(2, '0')}`);
                                ...
                                className="h-10 px-6 rounded-xl bg-slate-600 hover:bg-slate-700 text-white shadow-lg shadow-slate-500/20 text-[9px] font-black uppercase tracking-widest"
                            >
                                DISPENSAR TAREFA
                            </Button>
                        </div>
    ...
    return (
        <>
                    ...
                    <div className="flex-1 overflow-y-auto p-10 space-y-12">
                        ...
                        {filteredGuides.length === 0 ? (
                            ...
                            <div className="space-y-12">
                                ...
                                {/* DISPENSADAS */}
                                {groupedGuides.dismissed.length > 0 && (
                                    <div className="space-y-6">
                                        <h3 className="text-[10px] uppercase font-black tracking-[0.3em] text-slate-500/60 flex items-center gap-3 ml-4">
                                            <div className="w-1.5 h-1.5 rounded-full bg-slate-500" />
                                            Tarefas Dispensadas ({groupedGuides.dismissed.length})
                                        </h3>
                                        <div className="grid grid-cols-1 gap-4 opacity-70">
                                            {groupedGuides.dismissed.map(renderGuideItem)}
                                        </div>
                                    </div>
                                )}
                            </div>
            ...
            <Dialog open={isCompetencyDialogOpen} onOpenChange={setIsCompetencyDialogOpen}>
                <DialogContent className="max-w-md rounded-[2.5rem] border-none p-10 bg-card shadow-2xl">
                    <DialogHeader>
                        <DialogTitle className="text-2xl font-light">Competência</DialogTitle>
                        <DialogDescription className="text-xs uppercase font-bold tracking-widest pt-2">
                            Antes de dar baixa, confirme o mês de competência desta tarefa.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="flex gap-4">
                        ...
                        <Button
                            onClick={() => {
                                if (selectedGuideForBaixa && tempCompetency) {
                                    if (actionType === 'dispensa') {
                                        setIsCompetencyDialogOpen(false);
                                        setTempJustification('');
                                        setIsJustificationDialogOpen(true);
                                    } else {
                                        // Converte de yyyy-MM para MM/yyyy para salvar no banco
                                        const [y, m] = tempCompetency.split('-');
                                        const formattedComp = `${m}/${y}`;
                                        handleUpdateStatus(selectedGuideForBaixa, formattedComp);
                                        setIsCompetencyDialogOpen(false);
                                    }
                                }
                            }}
                            className={cn(
                                "flex-1 rounded-2xl h-14 text-[10px] uppercase font-black tracking-widest",
                                actionType === 'dispensa' ? "bg-slate-600 hover:bg-slate-700 text-white shadow-xl shadow-slate-500/20" : "bg-red-600 hover:bg-red-700 text-white shadow-xl shadow-red-500/20"
                            )}
                        >
                            {actionType === 'dispensa' ? "Continuar" : "Confirmar Baixa"}
                        </Button>
                    </div>
                </DialogContent>
            </Dialog>
            ...
            <Dialog open={isJustificationDialogOpen} onOpenChange={setIsJustificationDialogOpen}>
                <DialogContent className="max-w-md rounded-[2.5rem] border-none p-10 bg-card shadow-2xl">
                    <DialogHeader>
                        <DialogTitle className="text-2xl font-light">Justificativa</DialogTitle>
                        <DialogDescription className="text-xs uppercase font-bold tracking-widest pt-2 text-slate-500">
                            Por favor, descreva o motivo de estar dispensando esta tarefa.
                        </DialogDescription>
                    </DialogHeader>

                    <div className="space-y-6 py-6">
                        <div className="space-y-3">
                            <Label className="text-[10px] uppercase font-black tracking-widest opacity-60 ml-1">Motivo da Dispensa</Label>
                            <textarea
                                value={tempJustification}
                                onChange={(e) => setTempJustification(e.target.value)}
                                placeholder="Descreva aqui o motivo..."
                                className="w-full h-32 p-4 rounded-2xl border border-border/40 bg-muted/20 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary/20 resize-none"
                            />
                        </div>
                    </div>
                    ...
                    <div className="flex gap-4">
                        ...
                        <Button
                            onClick={() => {
                                if (selectedGuideForBaixa && tempJustification.trim()) {
                                    // Converte de yyyy-MM para MM/yyyy para salvar no banco
                                    const [y, m] = tempCompetency.split('-');
                                    const formattedComp = `${m}/${y}`;
                                    handleUpdateStatus(selectedGuideForBaixa, formattedComp, tempJustification);
                                    setIsJustificationDialogOpen(false);
                                } else {
                                    toast.error("Por favor, informe uma justificativa.");
                                }
                            }}
                            className="flex-1 rounded-2xl h-14 bg-slate-600 hover:bg-slate-700 text-white shadow-xl shadow-slate-500/20 text-[10px] uppercase font-black tracking-widest"
                        >
                            Dispensar Tarefa
                        </Button>
                    </div>
